...
def save_merged_data(merged_data: pd.DataFrame, output_dir: str, is_sample: bool = False):
    ...
    try:
        ...
        if 'home_team_name' in merged_data.columns and 'away_team_name' in merged_data.columns:
            print(f"\nExample matchups:")
            sample_games = merged_data.drop_duplicates('game_id')[['home_team_name', 'away_team_name', 'season']].head(5)
            for _, game in sample_games.iterrows():
                print(f"  {game['away_team_name']} @ {game['home_team_name']} ({game['season']})")
...
def main():
    ...
    parser.add_argument('--output_dir', default='results', help='Output directory for merged data')
