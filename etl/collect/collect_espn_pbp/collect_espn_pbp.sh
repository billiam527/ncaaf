...
echo "Running ESPN play-by-play collection script"
...
if [ $args -eq 0 ]; then
    ...
    for i in $(seq 2010 $last_complete_year); do
        ...
        if python3 run.py --start_date "$START_DATE" --end_date "$END_DATE"; then
            ...
            # Only exit if there's a real error (not just no data)
        fi
    done
    ...
    if ! python3 run.py --start_date "$START_DATE" --end_date "$END_DATE"; then
        echo "Error: Failed to collect data for year $year"
        exit 1
    fi
    ...
    for i in $(seq $start_year $end_year); do
        ...
        if ! python3 run.py --start_date "$START_DATE" --end_date "$END_DATE"; then
            echo "Error: Failed to collect data for year $i"
            exit 1
        fi
    done
...
if [ "$SKIP_S3_UPLOAD" = true ]; then
    ...
    if [ -d "temp" ]; then
        ...
        csv_count=$(find . -name "play-by-play*.csv" | wc -l)
        ...
        mkdir -p ../data/pbp/backups 2>/dev/null || true
        cp play-by-play*.csv ../data/pbp/backups/ 2>/dev/null || true
        echo "  Local backup: ../data/pbp/backups/"
    ...
    if [ -d "temp" ]; then
        ...
        csv_files=$(find . -name "play-by-play*.csv" | wc -l)
        csv_size=$(find . -name "play-by-play*.csv" -exec du -ch {} + 2>/dev/null | tail -1 | cut -f1 2>/dev/null || echo "0")
        
        echo "Essential files found:"
        echo "  CSV files: $csv_files (Size: $csv_size)"
        
        if [ $csv_files -gt 0 ]; then
            # Local backup first (fast)
            echo ""
            echo "1. Creating local backup..."
            mkdir -p ../data/pbp/backups
            cp play-by-play*.csv ../data/pbp/backups/
            echo "   SUCCESS: Local backup created in ../data/pbp/backups/"
            
            # S3 backup (essential data only)
            echo ""
            echo "2. Uploading essential data to S3..."
            echo "   Uploading $csv_files CSV file(s) ($csv_size total)..."
            
            if aws s3 cp . s3://ncaaf-data/espn-pbp-data/pbp/csvs/ --recursive --exclude "*" --include "play-by-play*.csv"; then
            ...
            json_count=$(find . -name "*.json" 2>/dev/null | wc -l || echo "0")
            other_count=$(find . -type f ! -name "play-by-play*.csv" ! -name "*.json" | wc -l)
            
            echo ""
            echo "Skipped uploads (saving time/bandwidth):"
            echo "  JSON files: $json_count (raw data, can be re-collected)"
            echo "  Other intermediate files: $other_count (can be regenerated)"
            
        else
            echo "WARNING: No essential CSV files found to backup"
        fi
        
        # Clean up non-essential files to save space
        echo ""
        echo "3. Cleaning up non-essential files..."
        rm -f dates_* date_urls* game_ids* gameid_urls* 2>/dev/null || true
        if [ -d "pbpjsons" ]; then
            json_size=$(du -sh pbpjsons 2>/dev/null | cut -f1 || echo "unknown")
        ...
        echo "  Local:  ../data/pbp/backups/"
        echo "  S3:     s3://ncaaf-data/espn-pbp-data/pbp/csvs/"
...
echo ""
echo "ESPN play-by-play collection completed successfully!"