...
NEW=['blue_chip_ratio_pct','top22_rating_pct','QB_rating_pct','OL_rating_pct',
     'DL_rating_pct','DB_rating_pct','mean_rating_pct']
Nf=RT.dropna(subset=['team_id'])[['team_id','season']+NEW].copy()
...
for side,idcol in (('home','home_team_id'),('away','away_team_id')):
    feat=feat.merge(Rf.rename(columns={c:f'{side}_{c}' for c in RET}),
                    ...
                    how='left').drop(columns=['team_id'])
    feat=feat.merge(Tf.rename(columns={'talent_roll_pct':f'{side}_talent'}),
                    left_on=[idcol,'season'],right_on=['team_id','season'],
                    how='left').drop(columns=['team_id'])
    feat=feat.merge(Nf.rename(columns={c:f'{side}_{c}' for c in NEW}),
                    left_on=[idcol,'season'],right_on=['team_id','season'],
                    how='left').drop(columns=['team_id'])

BASE=[c for c in feat.columns if 'adjusted' in c]
RETS=[f'{s}_{c}' for s in ('home','away') for c in RET]
OLD=[f'{s}_talent' for s in ('home','away')]
BC=[f'{s}_blue_chip_ratio_pct' for s in ('home','away')]
T22=[f'{s}_top22_rating_pct' for s in ('home','away')]
POS=[f'{s}_{c}' for s in ('home','away') for c in
     ('QB_rating_pct','OL_rating_pct','DL_rating_pct','DB_rating_pct')]
ALLNEW=[f'{s}_{c}' for s in ('home','away') for c in NEW]
for c in RETS+OLD+ALLNEW: feat[c]=feat[c].fillna(feat[c].median())
...
def walk(cols,label):
    ...
    print(f"  {label:<38}{len(cols):>4}f  MAE {np.abs(P-A).mean():.3f}  "
          f"side {((P>0)==(A>0)).mean():.1%}  | mkt corr "
          f"{np.corrcoef(P[ok],M[ok])[0,1]:+.3f}  |P-M| {np.abs(P[ok]-M[ok]).mean():.2f}")
    return P,A

print("\n=== walk-forward 2019-2025 ===")
b =walk(BASE+RETS+OLD,'shipped: class-points talent (88f)')
w1=walk(BASE+RETS+BC,'blue-chip ratio INSTEAD')
w2=walk(BASE+RETS+T22,'top-22 rating INSTEAD')
w3=walk(BASE+RETS+OLD+BC+T22,'class points + blue chip + top22')
w4=walk(BASE+RETS+OLD+ALLNEW,'class points + all roster measures')
w5=walk(BASE+RETS+OLD+BC+T22+POS,'+ position-group talent')

for lab,r in (('blue-chip instead',w1),('top22 instead',w2),
              ('points+bc+t22',w3),('points+all',w4),('+positions',w5)):
    d=np.abs(r[0]-r[1])-np.abs(b[0]-b[1]); se=d.std()/np.sqrt(len(d))
    print(f"  {lab:<22} MAE {np.abs(r[0]-r[1]).mean()-np.abs(b[0]-b[1]).mean():+.3f}"
          f"  (t={d.mean()/se:+.2f})")
PY
